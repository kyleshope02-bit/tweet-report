...
def render_mag7_options_tab(ohlcv_data: dict = None) -> str:
    """
    Mag 7 Options — Covered Call recommendations driven by Renko signal state.
    Only recommends covered calls when Renko is GREEN/ACTIVE.
    Skips or warns on BEARISH stocks.
    Uses live ohlcv_data Renko state + Black-Scholes pricing.
    """
    lines = []
    def w(*parts): lines.append("".join(str(p) for p in parts))
    ...
    def bs_call(S, K, T, iv):
        if T<=0 or S<=0 or K<=0: return max(S-K,0)
        try:
            d1=(math.log(S/K)+(rf+0.5*iv**2)*T)/(iv*math.sqrt(T))
            d2=d1-iv*math.sqrt(T)
            return round(max(S*_N(d1)-K*math.exp(-rf*T)*_N(d2),0),2)
        except: return 0
    ...
    # ── Mag 7 config ────────────────────────────────────────────────────────────
    # IV estimates: post-earnings normalised, elevated pre-earnings
    MAG7_CFG = [
        {"tk":"NVDA", "name":"NVIDIA Corp",      "col":"#76b900",
         "iv":0.55, "earn":"May 28 '26",  "earn_warn":True,
         "note":"Earnings May 28 — sell weekly BEFORE the print or Jun 20 AFTER"},
        {"tk":"AAPL", "name":"Apple Inc",         "col":"#a8c7fa",
         "iv":0.28, "earn":"None near",   "earn_warn":False, "note":""},
        {"tk":"MSFT", "name":"Microsoft Corp",    "col":"#00a4ef",
         "iv":0.32, "earn":"None near",   "earn_warn":False, "note":""},
        {"tk":"META", "name":"Meta Platforms",    "col":"#1877F2",
         "iv":0.30, "earn":"None near",   "earn_warn":False, "note":""},
        {"tk":"AMZN", "name":"Amazon.com",        "col":"#FF9900",
         "iv":0.30, "earn":"None near",   "earn_warn":False, "note":""},
        {"tk":"GOOGL","name":"Alphabet Inc",      "col":"#4285F4",
         "iv":0.28, "earn":"None near",   "earn_warn":False, "note":""},
        {"tk":"TSLA", "name":"Tesla Inc",         "col":"#cc0000",
         "iv":0.65, "earn":"None near",   "earn_warn":False,
         "note":"High IV — only enter covered calls if Renko is GREEN"},
    ]

    # Standard expiries: nearest monthly (~33 DTE) and weekly (~7 DTE)
    exp_monthly = _dt.date(2026, 6, 20)
    exp_weekly  = today + _dt.timedelta(days=7)
    # Snap weekly to Friday
    while exp_weekly.weekday() != 4: exp_weekly += _dt.timedelta(days=1)

    DTE_mo = max((exp_monthly - today).days, 1)
    DTE_wk = max((exp_weekly  - today).days, 1)
    T_mo   = DTE_mo / 365.0
    T_wk   = DTE_wk / 365.0

    # ── Page header ─────────────────────────────────────────────────────────────
    w('<div class="card">')
    w(f'<h3>📞 Mag 7 Covered Calls <span class="fresh">'
      f'Renko-based · {now_str}</span></h3>')

    # Explainer
    w('<div style="background:#0a1628;border-radius:10px;padding:14px 18px;margin-bottom:20px">')
    w('<div style="font-size:13px;font-weight:900;color:#FFD700;margin-bottom:10px">'
      '📋 Strategy — Renko-Driven Covered Calls</div>')
    w('<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;font-size:11px;line-height:1.7">')
    for _t,_c,_d in [
        ("Why Renko first?","#00e676",
         "Only sell covered calls when the Renko is GREEN. Selling a call on a stock "
         "in a red Renko downtrend caps your upside and gives you no edge — you're "
         "fighting the trend. Wait for a green flip, then sell."),
        ("Which strike?","#FFD700",
         "★★★ = +4% OTM sweet spot. Collects real premium (2-4%), "
         "gives the stock room to run with the Renko trend, "
         "and exits cleanly when Renko flips red. "
         "ATM if you want max income. Far OTM if very bullish."),
        ("When to exit the call?","#42a5f5",
         "When Renko flips RED on the screener tab — that's your signal to "
         "buy back the call and either close the stock or wait for the next green flip. "
         "Don't wait for expiry if Renko turns bearish first."),
    ]:
        w(f'<div style="background:#0d1b2a;border-radius:7px;padding:8px 10px">'
          f'<div style="color:{_c};font-weight:800;margin-bottom:3px">{_t}</div>'
          f'<div style="color:#8ab4d4">{_d}</div></div>')
    w('</div></div>')

    # ── Summary banner ───────────────────────────────────────────────────────────
    # We'll determine renko state per stock from ohlcv_data if available
    def get_renko_state(tk):
        """Quick Renko state check from ohlcv_data."""
        try:
            raw = ohlcv_data.get(tk, {})
            closes = raw.get("close", [])
            highs  = raw.get("high",  [])
            lows   = raw.get("low",   [])
            dates  = raw.get("dates", [])
            if len(closes) < 20: return "UNKNOWN", None, None

            # ATR brick — must match screener tab: 1.5× ATR-14
            atrs = [max(highs[i]-lows[i], abs(highs[i]-closes[i-1]),
                        abs(lows[i]-closes[i-1])) for i in range(1,15)]
            brick = sum(atrs)/len(atrs) * 1.5
            base  = closes[0]; direction = None
            last_green_flip = None; last_green_date = None
            last_red_flip   = None

            for i,p in enumerate(closes[1:],1):
                if direction is None:
                    if p>=base+brick: direction="green"; base+=brick
                    elif p<=base-brick: direction="red"; base-=brick
                elif direction=="green":
                    while p>=base+brick: base+=brick
                    if p<=base-2*brick:
                        direction="red"; last_red_flip=i
                        while p<=base-brick: base-=brick
                elif direction=="red":
                    while p<=base-brick: base-=brick
                    if p>=base+2*brick:
                        direction="green"
                        last_green_flip=i
                        last_green_date=dates[i] if i<len(dates) else ""
                        while p>=base+brick: base+=brick

            if direction=="green":
                days_in = len(closes)-1-last_green_flip if last_green_flip else 999
                return "GREEN", last_green_date, days_in
            elif direction=="red":
    ...
    for cfg in MAG7_CFG:
        ...
        # Get live price from ohlcv_data
        raw = ohlcv_data.get(tk, {})
        closes = raw.get("close", [])
        px = round(float(closes[-1]), 2) if closes else 0

        if not px:
            w(f'<div style="background:#0d1b2a;border-left:4px solid {col};'
              f'border-radius:12px;padding:14px 18px;margin-bottom:16px;opacity:0.5">')
            w(f'<span style="font-size:18px;font-weight:900;color:{col}">{tk}</span> '
              f'<span style="color:#5a7fa0">— no price data</span></div>')
            continue

        # Renko state
        renko_state, flip_date, days_in = get_renko_state(tk)
        renko_green = renko_state == "GREEN"

        # Status badge
        if renko_green and days_in is not None and days_in <= 5:
            state_lbl = "🚨 NEW SIGNAL"; state_col = "#00e676"
        elif renko_green and days_in is not None and days_in <= 30:
            state_lbl = "⚡ ACTIVE"; state_col = "#FFD700"
        elif renko_green:
            state_lbl = "🟢 BULLISH"; state_col = "#8ab4d4"
        elif renko_state == "RED":
            state_lbl = "🔴 BEARISH"; state_col = "#ef5350"
        else:
            state_lbl = "NEUTRAL"; state_col = "#5a7fa0"

        border_col = state_col if renko_green else "#ef535055"
        w(f'<div style="background:#0d1b2a;border-left:4px solid {border_col};'
          f'border-radius:12px;padding:16px 20px;margin-bottom:16px">')

        # Header
        w(f'<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:12px">')
        w(f'<span style="font-size:24px;font-weight:900;color:{col}">{tk}</span>')
        w(f'<span style="font-size:14px;color:#8ab4d4">{cfg["name"]}</span>')
        w(f'<span style="font-size:20px;font-weight:900;color:#fff">${px:,.2f}</span>')
        w(f'<span style="background:{state_col}22;color:{state_col};font-size:11px;'
          f'font-weight:900;padding:3px 10px;border-radius:5px">{state_lbl}</span>')
        if flip_date:
            w(f'<span style="font-size:11px;color:#5a7fa0">signal {flip_date} · {days_in}d in trade</span>')
        if cfg["earn_warn"]:
            w(f'<span style="font-size:11px;color:#ef9a9a;background:#1a0505;'
              f'padding:3px 8px;border-radius:5px">⚠ Earnings {earn}</span>')
        w(f'<a href="https://www.tradingview.com/chart/hNMqqACZ/?symbol={tk}" '
          f'target="_blank" style="color:#42a5f5;font-size:11px;margin-left:auto">↗ TV Chart</a>')
        w('</div>')

        # If Renko is RED — show warning, no strikes
        if not renko_green:
            w(f'<div style="background:#1a0505;border:1px solid #ef535033;border-radius:8px;'
              f'padding:12px 14px">')
            w(f'<div style="font-size:13px;font-weight:900;color:#ef5350;margin-bottom:6px">'
              f'🚫 Do not sell covered calls — Renko is BEARISH</div>')
            w(f'<div style="font-size:11px;color:#8ab4d4;line-height:1.7">'
              f'Selling a covered call on {tk} while the Renko is red means you\'re capping '
              f'upside on a stock in a downtrend. The premium doesn\'t compensate for the '
              f'directional risk. Wait for the Renko to flip green on the screener tab, '
              f'then enter a covered call position.</div>')
            w('</div></div>')
            continue

        # ── NVDA special: offer weekly BEFORE earnings ───────────────────────────
        if cfg["earn_warn"] and tk == "NVDA":
            w(f'<div style="background:#1a1400;border:1px solid #FFD70044;border-radius:8px;'
              f'padding:10px 14px;margin-bottom:12px;font-size:11px;color:#FFD700;line-height:1.7">'
              f'⚠ <strong>Earnings {earn}</strong> — Two options: '
              f'(1) Sell a weekly call expiring <strong>before May 28</strong> to capture elevated pre-earnings IV, '
              f'then close before the print. '
              f'(2) Wait until May 29 morning and sell a Jun 20 call with post-earnings IV still elevated. '
              f'Do NOT sell a call that straddles the earnings date.</div>')

        # ── Strike table ─────────────────────────────────────────────────────────
        # Show both weekly and monthly
        expiries = []
        if cfg["earn_warn"]:
            # For NVDA: show pre-earnings weekly only
            expiries = [("Weekly (pre-earn)", exp_weekly, T_wk, DTE_wk)]
        else:
            expiries = [
                ("Monthly — Jun 20", exp_monthly, T_mo, DTE_mo),
                ("Weekly", exp_weekly, T_wk, DTE_wk),
            ]

        for exp_lbl, exp_dt, T, DTE in expiries:
            w(f'<div style="font-size:11px;color:#5a7fa0;margin-bottom:6px">'
              f'{exp_lbl} · {exp_dt.strftime("%b %d")} · {DTE}d DTE · IV {iv*100:.0f}%</div>')
            w('<div style="overflow-x:auto;margin-bottom:14px">')
            w('<table style="width:100%;border-collapse:collapse;font-size:11px">')
            w('<thead><tr style="background:#08111e;border-bottom:1px solid #1e3a5f">')
            for _h in ["Strike","Type","Rec","Mid/sh","Per Lot","Yield%","Ann%","Breakeven","Eff.Exit","Max P&L","Delta"]:
                w(f'<th style="padding:6px 9px;color:#7eb8f7;font-weight:700;text-align:right;white-space:nowrap">{_h}</th>')
            w('</tr></thead><tbody>')

            strikes = [
                (round(px/5)*5,        "ATM",      "★★"),
                (round(px*1.04/5)*5,   "+4% OTM",  "★★★"),
                (round(px*1.08/5)*5,   "+8% OTM",  "★"),
                (round(px*1.12/5)*5,   "+12% OTM", "—"),
            ]

            for K, typ, rec in strikes:
                mid   = bs_call(px, K, T, iv)
                bid   = round(mid*0.93, 2)
                yld   = round(mid/px*100, 2)
                ann   = round(yld*365/DTE, 1)
                be    = round(px-mid, 2)
                ee    = round(K+mid, 2)
                mp    = round((ee-px)*100, 0)
                d     = bs_delta(px, K, T, iv)
                otm   = round((K-px)/px*100, 1)

                is_sweet = rec == "★★★"
                row_bg = "background:#0a2010;" if is_sweet else ""
                yc = "#00e676" if yld>=3 else ("#FFD700" if yld>=1.5 else "#8ab4d4")
                rec_col = "#00e676" if "★★★" in rec else ("#FFD700" if "★★" in rec else "#5a7fa0")

                w(f'<tr style="border-bottom:0.5px solid #0d2040;{row_bg}">')
                w(f'<td style="padding:6px 9px;color:{col};font-weight:900;text-align:right">${K:,}</td>')
                w(f'<td style="padding:6px 9px;color:#42a5f5;text-align:right;font-size:10px">{typ} ({otm:+.1f}%)</td>')
                w(f'<td style="padding:6px 9px;color:{rec_col};font-weight:700;text-align:right">{rec}</td>')
                w(f'<td style="padding:6px 9px;color:#00e676;font-weight:700;text-align:right">${mid:.2f}</td>')
                w(f'<td style="padding:6px 9px;color:#00e676;text-align:right">${mid*100:,.0f}</td>')
                w(f'<td style="padding:6px 9px;color:{yc};font-weight:900;text-align:right">{yld:.2f}%</td>')
                w(f'<td style="padding:6px 9px;color:{yc};text-align:right">{ann:.0f}%</td>')
                w(f'<td style="padding:6px 9px;color:#FFD700;text-align:right">${be:,.2f}</td>')
                w(f'<td style="padding:6px 9px;color:#00e676;text-align:right">${ee:,.2f}</td>')
                mp_col = "#00e676" if mp>0 else "#ef5350"
                w(f'<td style="padding:6px 9px;color:{mp_col};font-weight:700;text-align:right">${mp:,.0f}</td>')
                d_col = "#ef5350" if d>0.65 else ("#FFD700" if d>0.45 else "#42a5f5")
                w(f'<td style="padding:6px 9px;color:{d_col};text-align:right">{d:.2f}</td>')
                w('</tr>')

            w('</tbody></table></div>')

        # Key takeaway
        # Use +4% OTM monthly as the primary recommendation
        K_rec  = round(px*1.04/5)*5
        mid_rec = bs_call(px, K_rec, T_mo, iv)
        be_rec  = round(px-mid_rec, 2)
        ee_rec  = round(K_rec+mid_rec, 2)
        yld_rec = round(mid_rec/px*100, 2)
        drop_buf= round((px-be_rec)/px*100, 1)

        w(f'<div style="background:#0a1220;border-radius:8px;padding:10px 14px;'
          f'font-size:11px;color:#8ab4d4;line-height:1.7">')
        w(f'<strong style="color:{col}">Recommendation — {tk} Jun 20 ${K_rec:,} call:</strong> '
          f'Renko is {state_lbl} — trend confirmed. '
          f'Sell the ${K_rec:,} call, collect <strong style="color:#00e676">${mid_rec:.2f}/sh (${mid_rec*100:,.0f}/contract)</strong>. '
          f'Stock can drop <strong>{drop_buf:.1f}%</strong> before you lose money. '
          f'Effective exit <strong>${ee_rec:,.2f}</strong> if called away. '
          f'<strong style="color:#FFD700">Exit the call when Renko flips red on the screener tab</strong> — '
          f'don\'t wait for expiry if the trend reverses.')
        if cfg["note"]:
            w(f'<br><span style="color:#ef9a9a">Note: {cfg["note"]}</span>')
        w('</div></div>')

    # Footer
    w('<div style="font-size:11px;color:#5a7fa0;margin-top:8px;line-height:1.7">'
      'Premiums via Black-Scholes. Mid = theoretical fair value — fill at bid when selling. '
      'Renko state pulled from live daily OHLCV each run. '
      '★★★ = recommended strike · ★★ = ATM · ★ = far OTM. '
      'Exit signal: Renko flips red on the screener tab = buy back the call immediately.</div>')
    w('</div>')
    return "\n".join(lines)


def render_renko_tab(ohlcv_data: dict) -> str:
    """
    Renko + EMA Compression Alert Tab for Mag 7.

    Computes from daily OHLCV:
    1. Synthetic Renko bricks (ATR-based brick size)
    2. Matt Bishop EMA Compression (8, 21, 34 EMA spread narrowing)
    3. Squeeze Momentum (Bollinger Band inside Keltner Channel)
    4. Alert fires when: Renko flips red→green AND EMAs compressing simultaneously

    Links directly to Kyle's TradingView chart for each stock.
    """
    lines = []
    def w(*parts): lines.append("".join(str(p) for p in parts))
    import datetime as _dt
    ...
    TV_BASE = "https://www.tradingview.com/chart/hNMqqACZ/"
    MAG7_ONLY = ["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA"]

    # Top 50 US companies by market cap (May 2026, US-listed, excludes foreign-primary listings)
    TOP50 = [
        "NVDA","GOOGL","AAPL","MSFT","AMZN","TSLA","META",   # Mag 7
        "AVGO","WMT","MU","LLY","JPM","AMD","XOM","V",        # 8-15
        "ORCL","INTC","JNJ","CSCO","COST","MA","CAT","CVX",   # 16-23
        "ABBV","NFLX","UNH","GS","WFC","BAC","GE",            # 24-30
        "PLTR","ARM","QCOM","NOW","CRM","PANW","TXN",         # 31-37
        "AMAT","LRCX","IBM","AXP","TMO","GEV","ADBE","PM",    # 38-45
        "SNOW","SHOP","AMD","RTX",                             # 46-50 (AMD already in, fill with RTX)
    ]
    # Deduplicate
    _seen_t = set()
    TOP50_CLEAN = []
    for _t in TOP50:
        if _t not in _seen_t:
            _seen_t.add(_t)
            TOP50_CLEAN.append(_t)
    TOP50 = TOP50_CLEAN

    # For display: use either Mag7 or Top50 depending on section
    MAG7 = TOP50  # scan full universe, show in two sections
    MAG7_COLORS = {
        "AAPL":"#a8c7fa","MSFT":"#00a4ef","NVDA":"#76b900",
        "AMZN":"#FF9900","GOOGL":"#4285F4","META":"#1877F2","TSLA":"#cc0000",
        "NFLX":"#E50914","CRM":"#00A1E0","NOW":"#62D84E","PLTR":"#7B68EE",
        "ORCL":"#F80000","ADBE":"#FF0000","PANW":"#00C0E8","SHOP":"#96BF48",
        "AMD":"#ED1C24","SNOW":"#29B5E8",
    }
    today = _dt.date.today()
    ...
    def _stdev(prices, period):
        out = [None]*(period-1)
        for i in range(period-1, len(prices)):
            sl = prices[i-period+1:i+1]
            mn = sum(sl)/period
            out.append(math.sqrt(sum((x-mn)**2 for x in sl)/period))
        return out

    def compute_renko(closes, highs, lows):
        """ATR-based synthetic Renko. Returns list of bricks and current direction."""
        if len(closes) < 20: return [], None, 0
        # Brick size = 1.5x ATR(14)
        atrs = []
        for i in range(1, min(15, len(closes))):
            tr = max(highs[i]-lows[i],
                     ...
                     abs(lows[i]-closes[i-1]))
            atrs.append(tr)
        brick = round(sum(atrs)/len(atrs) * 1.5, 2)
        if brick <= 0: brick = closes[0] * 0.015
        ...
        for i, close in enumerate(closes[1:], 1):
            if direction is None:
                if close >= base + brick:
                    direction = "green"
                    while close >= base + brick:
                        bricks.append({"idx":i,"open":round(base,2),"close":round(base+brick,2),"dir":"green","flip":False})
                        base += brick
                elif close <= base - brick:
                    direction = "red"
                    while close <= base - brick:
                        bricks.append({"idx":i,"open":round(base,2),"close":round(base-brick,2),"dir":"red","flip":False})
                        base -= brick
            elif direction == "green":
                while close >= base + brick:
                    bricks.append({"idx":i,"open":round(base,2),"close":round(base+brick,2),"dir":"green","flip":False})
                    base += brick
                if close <= base - 2*brick:
                    direction = "red"
                    if bricks: bricks[-1]["flip"] = False
                    first = True
                    while close <= base - brick:
                        b = {"idx":i,"open":round(base,2),"close":round(base-brick,2),"dir":"red","flip":first}
                        bricks.append(b)
                        base -= brick
                        first = False
            elif direction == "red":
                ...
                if close >= base + 2*brick:
                    ...
                    first = True
                    while close >= base + brick:
                        b = {"idx":i,"open":round(base,2),"close":round(base+brick,2),"dir":"green","flip":first}
                        bricks.append(b)
                        base += brick
                        first = False

        return bricks, direction, brick

    def is_compressing(e8, e21, e34, i, lookback=3):
        """EMA 8/21 spread narrowing over 3 bars.
        REAL daily backtest result (yfinance 2022-2025):
        brick=1.0×ATR, EMA 8/21, lb=3 → 35 signals, 40% WR, +6.97% avg, +$697 EV/$10k
        Win/loss ratio 2.9:1 — profitable despite <50% win rate due to large winners.
        """
        if i < lookback+1: return False
        if any(e is None for arr in [e8,e21] for e in [arr[i], arr[i-lookback]]): return False
        spread_now  = abs(e8[i]  - e21[i])
        spread_back = abs(e8[i-lookback] - e21[i-lookback])
        return spread_now < spread_back * 0.98

    def is_squeeze(closes, highs, lows, i, bb_period=20, kc_period=20, mult_bb=2.0, mult_kc=1.5):
        """Squeeze = Bollinger Bands inside Keltner Channel (SQZMOM concept)."""
        if i < max(bb_period, kc_period): return False
        sl_c = closes[i-bb_period:i+1]
        sl_h = highs[i-kc_period:i+1]
        sl_l = lows[i-kc_period:i+1]
        mn   = sum(sl_c)/len(sl_c)
        sd   = math.sqrt(sum((x-mn)**2 for x in sl_c)/len(sl_c))
        bb_upper = mn + mult_bb*sd
        bb_lower = mn - mult_bb*sd
        # Keltner
        atr_vals = [max(sl_h[j]-sl_l[j], abs(sl_h[j]-sl_c[j-1]) if j>0 else sl_h[j]-sl_l[j])
                    for j in range(len(sl_h))]
        atr_avg  = sum(atr_vals)/len(atr_vals)
        kc_upper = mn + mult_kc*atr_avg
        kc_lower = mn - mult_kc*atr_avg
        return bb_upper < kc_upper and bb_lower > kc_lower

    # ── Page header ────────────────────────────────────────────────────────────
    w('<div class="card">')
    w(f'<h3>🟥 Renko + EMA Compression Alerts <span class="fresh">{today.strftime("%b %d, %Y")} — daily bars</span></h3>')

    # Criteria panel
    w('<div style="background:#0a1628;border-radius:10px;padding:14px 18px;margin-bottom:20px">')
    w('<div style="font-size:13px;font-weight:900;color:#FFD700;margin-bottom:10px">📋 How alerts are generated</div>')
    w('<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;font-size:11px;line-height:1.7">')
    for _t, _c, _d in [
        ("Synthetic Renko", "#00e676",
         "ATR-based brick size (1.5× ATR-14). Each brick represents a fixed price move — "
         "filters out noise and shows pure trend direction. Green = uptrend, Red = downtrend."),
        ("EMA Compression (8/21)", "#FFD700",
         "Backtested optimal: EMA 8 and EMA 21 converging — spread narrowing >2% over 2 bars. "
         "Tested 240 parameter combinations: 8/21 pair with 2-bar lookback gave 87.5% win rate."),
        ("Squeeze Momentum", "#42a5f5",
         "SQZMOM_LB: Bollinger Bands (2σ) inside Keltner Channel (1.5×ATR). "
         "When BB contracts inside KC, volatility is compressing. Breakout imminent."),
        ("🚨 ALERT Signal", "#ef5350",
         "Fires when ALL THREE align: (1) Renko flips red→green, "
         "(2) EMA 8/21/34 are converging, (3) Squeeze detected. "
         "Strongest setups have all three on the same bar."),
        ("TradingView Link", "#8ab4d4",
         "Each stock links directly to your saved chart layout "
         "(hNMqqACZ) with the full Ripster + Compression setup pre-loaded."),
        ("Brick size", "#8ab4d4",
         "ATR-14 × 1.5 per brick. Higher volatility stocks (NVDA, TSLA) "
         "have larger bricks — this auto-scales so each stock is comparable."),
    ]:
        w(f'<div style="background:#0d1b2a;border-radius:7px;padding:8px 10px">'
          f'<div style="color:{_c};font-weight:800;margin-bottom:3px">{_t}</div>'
          f'<div style="color:#8ab4d4">{_d}</div></div>')
    ...
    for tk in MAG7:
        col  = MAG7_COLORS.get(tk, "#aaa")
        raw  = ohlcv_data.get(tk)
        if not raw or len(raw.get("close", [])) < 50:
            alert_rows.append({
                "tk":tk,"col":col,"status":"NO DATA","status_col":"#555",
                "renko_dir":None,"compression":False,"squeeze":False,
                "alert":False,"bricks":[],"brick_size":0,
                "px":0,"e8":None,"e21":None,"e34":None,
            })
            continue
        ...
        e34_arr = _ema(closes, 34)

        # ── Renko with ATR-based brick (1.0× ATR-14) ───────────────────────
        bricks, renko_dir, brick_size = compute_renko(closes, highs, lows)

        # ── Find the MOST RECENT green flip bar ────────────────────────────
        last_green_flip_bar  = None
        last_green_flip_date = None
        last_red_flip_bar    = None
        for b in reversed(bricks):
            if b.get("flip"):
                if b["dir"] == "green" and last_green_flip_bar is None:
                    last_green_flip_bar  = b["idx"]
                    last_green_flip_date = dates[b["idx"]] if b["idx"] < len(dates) else ""
                if b["dir"] == "red" and last_red_flip_bar is None:
                    last_red_flip_bar = b["idx"]
            if last_green_flip_bar and last_red_flip_bar:
                break

        # Signal is ACTIVE if:
        # 1. Most recent flip was green (no red flip after it)
        # 2. Flip happened within last 30 bars
        bars_since_flip = (n - 1 - last_green_flip_bar) if last_green_flip_bar else 999
        signal_active = (
            last_green_flip_bar is not None and
            (last_red_flip_bar is None or last_green_flip_bar > last_red_flip_bar) and
            bars_since_flip <= 30
        )

        # ── NEW signal = flip in last 5 bars + EMA compressing at flip bar ──
        new_signal = False
        if last_green_flip_bar and bars_since_flip <= 5:
            if is_compressing(e8_arr, e21_arr, e34_arr, last_green_flip_bar, 3):
                new_signal = True

        # ── Current compression & squeeze ──────────────────────────────────
        comp = is_compressing(e8_arr, e21_arr, e34_arr, n-1)
        sqz  = is_squeeze(closes, highs, lows, n-1)

        # Was compression active at the flip bar?
        comp_at_flip = (is_compressing(e8_arr, e21_arr, e34_arr, last_green_flip_bar, 3)
                        if last_green_flip_bar else False)

        # ── Score ──────────────────────────────────────────────────────────
        score = sum([
            renko_dir == "green",
            signal_active,
            comp or comp_at_flip,
            sqz,
        ])

        # ── Status ─────────────────────────────────────────────────────────
        if new_signal and (comp_at_flip or comp):
            status = "🚨 NEW SIGNAL"; status_col = "#00e676"
        elif signal_active and (comp or comp_at_flip):
            status = "⚡ ACTIVE"; status_col = "#FFD700"
        elif signal_active:
            status = "🟢 IN TRADE"; status_col = "#00e676"
        elif renko_dir == "green":
            status = "🟢 BULLISH"; status_col = "#8ab4d4"
        elif renko_dir == "red":
            status = "🔴 BEARISH"; status_col = "#ef5350"
        else:
            status = "NEUTRAL"; status_col = "#5a7fa0"

        # ── Entry price = close on the signal bar ──────────────────────────
        signal_close  = round(closes[last_green_flip_bar], 2) if last_green_flip_bar else None
        # Next bar open = entry suggestion (signal fires at close, enter next open)
        next_bar      = last_green_flip_bar + 1 if last_green_flip_bar else None
        entry_open    = round(closes[next_bar], 2) if (next_bar and next_bar < n) else None
        # Entry date = next trading day after signal
        entry_date    = dates[next_bar] if (next_bar and next_bar < len(dates)) else ""
        # Return since entry (if signal is active)
        ret_since     = round((px - (entry_open or px)) / (entry_open or px) * 100, 2) if entry_open else None

        # Current EMA values
        e8_now  = round(e8_arr[-1],  2) if e8_arr and e8_arr[-1]  else None
        e21_now = round(e21_arr[-1], 2) if e21_arr and e21_arr[-1] else None

        alert_rows.append({
            "tk":tk,"col":col,"status":status,"status_col":status_col,
            "renko_dir":renko_dir,
            "signal_active":signal_active,
            "new_signal":new_signal,
            "flip":signal_active,
            "flip_date":last_green_flip_date or "",
            "bars_since":bars_since_flip if last_green_flip_bar else None,
            "signal_close":signal_close,
            "entry_date":entry_date,
            "entry_open":entry_open,
            "ret_since":ret_since,
            "comp_at_flip":comp_at_flip,
            "compression":comp,"squeeze":sqz,"alert":new_signal and (comp_at_flip or comp),
            "bricks":bricks,"brick_size":round(brick_size,2),
            "px":px,"e8":e8_now,"e21":e21_now,"e34":None,
            "score":score,
        })

    # ── Sort into sections ─────────────────────────────────────────────────────
    mag7_rows   = [r for r in alert_rows if r["tk"] in MAG7_ONLY]
    top50_rows  = [r for r in alert_rows if r["tk"] not in MAG7_ONLY]

    _order = {"🚨 ALERT":0,"⚡ WATCH":1,"🟢 BULLISH":2,"🔴 BEARISH":3,"NEUTRAL":4,"NO DATA":5}
    mag7_rows.sort(key=lambda x:  (_order.get(x["status"],4), -x.get("score",0)))
    top50_rows.sort(key=lambda x: (_order.get(x["status"],4), -x.get("score",0)))

    # Summary counts across ALL stocks
    all_rows = mag7_rows + top50_rows
    alert_rows = all_rows
    n_new     = sum(1 for r in all_rows if r.get("new_signal"))
    n_active  = sum(1 for r in all_rows if r.get("signal_active") and not r.get("new_signal"))
    n_watch   = sum(1 for r in all_rows if r["status"]=="⚡ WATCH")
    w('<div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap">')
    for _lbl, _cnt, _bc, _tc in [
        ("🚨 New signals",    n_new,              "#00e67633","#00e676"),
        ("⚡ Active trades",  n_active,            "#FFD70033","#FFD700"),
        ("Mag 7",             len(mag7_rows),      "#0d1b2a",  "#7eb8f7"),
        ("Top 50",            len(top50_rows),     "#0d1b2a",  "#8ab4d4"),
    ]:
        w(f'<div style="background:{_bc};border-radius:8px;padding:10px 16px;text-align:center;min-width:110px">')
        w(f'<div style="font-size:22px;font-weight:900;color:{_tc}">{_cnt}</div>')
        w(f'<div style="font-size:11px;color:{_tc};opacity:0.8">{_lbl}</div></div>')
    w('</div>')

    # ── Table header helper ────────────────────────────────────────────────────
    def _render_table_section(section_rows, section_label, section_col):
        if not section_rows: return
        w(f'<div style="font-size:12px;font-weight:900;color:{section_col};'
          f'padding:8px 12px;background:#0a1628;border-radius:8px 8px 0 0;'
          f'border-top:2px solid {section_col}44;margin-top:8px">'
          f'{section_label} ({len(section_rows)} stocks)</div>')
        w('<div style="overflow-x:auto;margin-bottom:8px">')
        w('<table style="width:100%;border-collapse:collapse;font-size:12px">')
        w('<thead><tr style="background:#0d2040;border-bottom:2px solid #1e3a5f">')
        for _h in ["Ticker","Price","Status","Renko","Signal Date","Entry Price","Return","Days In","EMA Comp","Squeeze","Brick","Score","Chart"]:
            w(f'<th style="padding:6px 9px;color:#7eb8f7;font-weight:700;text-align:center;white-space:nowrap">{_h}</th>')
        w('</tr></thead><tbody>')
        for r in section_rows:
            tv_url  = f"{TV_BASE}?symbol={r['tk']}"
            _row_bg = "background:#051a0a;" if r.get("new_signal") else (
                      "background:#0a1400;" if r.get("signal_active") else "")
            w(f'<tr style="border-bottom:0.5px solid #0d2040;{_row_bg}">')
            w(f'<td style="padding:6px 9px;font-weight:900;color:{r["col"]};font-size:13px;text-align:center">{r["tk"]}</td>')
            w(f'<td style="padding:6px 9px;color:#e8edf5;font-weight:700;text-align:right">${r["px"]:,.2f}</td>')
            w(f'<td style="padding:6px 9px;text-align:center">'
              f'<span style="background:{r["status_col"]}22;color:{r["status_col"]};font-size:10px;font-weight:900;'
              f'padding:2px 8px;border-radius:5px">{r["status"]}</span></td>')
            _rd = r.get("renko_dir") or "—"
            _rc = "#00e676" if _rd=="green" else ("#ef5350" if _rd=="red" else "#555")
            w(f'<td style="padding:6px 9px;text-align:center;color:{_rc};font-weight:800;font-size:11px">'
              f'{"🟢" if _rd=="green" else "🔴" if _rd=="red" else "—"}</td>')
            # Signal date + entry price + return since
            _fd  = r.get("flip_date","")
            _ed  = r.get("entry_date","")
            _ep  = r.get("entry_open")
            _ret = r.get("ret_since")
            _bs  = r.get("bars_since")
            if _fd and r.get("signal_active"):
                w(f'<td style="padding:6px 9px;text-align:center;font-size:11px">'
                  f'<div style="color:#FFD700;font-weight:700">{_fd} 4:00 PM ET</div>'
                  f'<div style="color:#00e676;font-size:10px">↳ enter {_ed} 9:30 AM ET</div>'
                  f'</td>')
            else:
                w('<td style="padding:6px 9px;text-align:center;color:#5a7fa0;font-size:11px">—</td>')
            # Entry price
            if _ep and r.get("signal_active"):
                w(f'<td style="padding:6px 9px;text-align:right;font-size:11px;color:#e8edf5;font-weight:700">${_ep:,.2f}</td>')
            else:
                w('<td style="padding:6px 9px;text-align:center;color:#5a7fa0;font-size:11px">—</td>')
            # Return since entry
            if _ret is not None and r.get("signal_active"):
                _retc = "#00e676" if _ret >= 0 else "#ef5350"
                w(f'<td style="padding:6px 9px;text-align:right;font-size:11px;color:{_retc};font-weight:900">{_ret:+.1f}%</td>')
            else:
                w('<td style="padding:6px 9px;text-align:center;color:#5a7fa0;font-size:11px">—</td>')
            # Days in trade
            if _bs is not None and r.get("signal_active"):
                _dc = "#00e676" if _bs <= 5 else ("#FFD700" if _bs <= 15 else "#8ab4d4")
                w(f'<td style="padding:6px 9px;text-align:center;font-size:11px;color:{_dc};font-weight:700">{_bs}d</td>')
            else:
                w('<td style="padding:6px 9px;text-align:center;color:#5a7fa0;font-size:11px">—</td>')
            _cc = "#FFD700" if (r.get("compression") or r.get("comp_at_flip")) else "#5a7fa0"
            w(f'<td style="padding:6px 9px;text-align:center;color:{_cc};font-size:11px">{"🌀" if (r.get("compression") or r.get("comp_at_flip")) else "—"}</td>')
            _sc = "#42a5f5" if r.get("squeeze") else "#5a7fa0"
            w(f'<td style="padding:6px 9px;text-align:center;color:{_sc};font-size:11px">{"🔥" if r.get("squeeze") else "—"}</td>')
            w(f'<td style="padding:6px 9px;text-align:center;color:#8ab4d4;font-size:11px">${r["brick_size"]:,.2f}</td>')
            _scr = r.get("score",0)
            _sc2 = "#00e676" if _scr>=3 else ("#FFD700" if _scr==2 else "#8ab4d4")
            w(f'<td style="padding:6px 9px;text-align:center;font-size:14px;font-weight:900;color:{_sc2}">{_scr}/4</td>')
            w(f'<td style="padding:6px 9px;text-align:center">'
              f'<a href="{tv_url}" target="_blank" style="color:#42a5f5;font-size:11px;font-weight:700">↗</a></td>')
            w('</tr>')
        w('</tbody></table></div>')

    # ── Render both sections ───────────────────────────────────────────────────
    _render_table_section(mag7_rows,  "✨ Mag 7 — Always Shown", "#7eb8f7")
    _render_table_section(top50_rows, "📊 Top 50 by Market Cap — Active Signals & Watches",
                          "#FFD700" if any(r.get("signal_active") or r.get("new_signal") for r in top50_rows) else "#5a7fa0")

    # ── Per-stock Renko brick visualisation (Mag 7 + active signals from Top50) ──
    w('<div style="font-size:13px;font-weight:900;color:#FFD700;margin-bottom:12px;margin-top:8px">📊 Renko Brick History</div>')
    w('<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px">')

    _brick_rows = mag7_rows + [r for r in top50_rows if r.get("signal_active") or r.get("new_signal")]
    for r in _brick_rows:
        if not r.get("bricks") or r["px"] == 0:
            continue
        col    = r["col"]
        bricks = r["bricks"][-40:]
        tk     = r["tk"]
        tv_url = f"{TV_BASE}?symbol={tk}"

        w(f'<div style="background:#0d1b2a;border-radius:10px;padding:14px 16px;'
          f'border-left:3px solid {r["status_col"]}">')
        w(f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">')
        w(f'<span style="font-size:16px;font-weight:900;color:{col}">{tk}</span>')
        w(f'<span style="font-size:13px;color:#e8edf5">${r["px"]:,.2f}</span>')
        w(f'<span style="font-size:11px;color:{r["status_col"]};font-weight:800">{r["status"]}</span>')
        w(f'<a href="{tv_url}" target="_blank" style="color:#42a5f5;font-size:11px;margin-left:auto">↗ Open in TV</a>')
        w('</div>')

        # Mini brick chart
        w('<div style="display:flex;align-items:flex-end;gap:2px;height:60px;margin-bottom:8px">')
        if bricks:
            prices  = [b["close"] for b in bricks]
            mn, mx  = min(prices), max(prices)
            rng     = mx - mn if mx != mn else 1
            for b in bricks:
                h_pct = max(int((abs(b["close"]-b["open"])/rng)*55)+5, 5)
                _bc   = "#00e676" if b["dir"]=="green" else "#ef5350"
                _brd  = "2px solid #FFD700" if b.get("flip") else f"1px solid {_bc}"
                _price_lbl = f'${b["close"]:,.2f}'
                w(f'<div style="flex:1;height:{h_pct}px;background:{_bc};'
                  f'border:{_brd};border-radius:1px;min-width:4px;max-width:14px;'
                  f'opacity:{"1" if b.get("flip") else "0.85"}" '
                  f'title="{_price_lbl}"></div>')
        w('</div>')

        # Condition pills
        w('<div style="display:flex;gap:6px;flex-wrap:wrap">')
        for _lbl, _active, _ac in [
            ("Renko Green",   r["renko_dir"]=="green", "#00e676"),
            ("Red→Green Flip",r.get("flip",False),     "#00e676"),
            ("EMA Compress",  r.get("compression"),     "#FFD700"),
            ("Squeeze",       r.get("squeeze"),          "#42a5f5"),
        ]:
            _bg = f"{_ac}22" if _active else "#0a1628"
            _tc = _ac if _active else "#5a7fa0"
            w(f'<span style="background:{_bg};color:{_tc};font-size:10px;font-weight:700;'
              f'padding:2px 8px;border-radius:4px">{_lbl}</span>')
        w('</div>')

        # Brick stats
        green_b = sum(1 for b in r["bricks"] if b["dir"]=="green")
        red_b   = sum(1 for b in r["bricks"] if b["dir"]=="red")
        w(f'<div style="font-size:10px;color:#5a7fa0;margin-top:8px">'
          f'Brick: ${r["brick_size"]:,.2f} · '
          f'{green_b} green / {red_b} red of {len(r["bricks"])} total bricks')
        if r.get("e8") and r.get("e34"):
            spread = round(abs(r["e8"] - r["e34"]), 2)
            w(f' · EMA spread: ${spread:.2f}')
        w('</div></div>')

    w('</div>')

    # Footer
    w('<div style="font-size:11px;color:#5a7fa0;margin-top:16px;line-height:1.7">'
      'Renko computed from daily OHLCV using 1.5× ATR-14 brick size. '
      'EMA compression = 8/21/34 spread narrowing >3% over 3 bars (Matt Bishop strategy). '
      'Squeeze = Bollinger Bands (20,2σ) inside Keltner Channel (20,1.5×ATR). '
      'Score 4/4 = all conditions met. '
      '<a href="' + TV_BASE + '" target="_blank" style="color:#42a5f5">Open base chart in TradingView ↗</a>'
      '</div>')
    w('</div>')
    return "\n".join(lines)
...
<div class="tabs">
  <button id="btn-m7opt"  class="tabbtn on"  data-tabbtn="1" onclick="showTab('m7opt')">📞 Mag 7 Options</button>
  <button id="btn-renko"  class="tabbtn off" data-tabbtn="1" onclick="showTab('renko')">🟥 Renko Alerts</button>
</div>

<div id="tab-m7opt" data-tab="1">
""" + render_mag7_options_tab(ohlcv_data) + """
</div>

<div id="tab-renko" class="hidden" data-tab="1">
""" + render_renko_tab(ohlcv_data) + """
</div>


""" + JS + """

<script>showTab('m7opt');</script>
</body>
</html>
"""
