...
def render_renko_tab(ohlcv_data: dict) -> str:
    ...
    def _render_table_section(section_rows, section_label, section_col):
        ...
        w('<table style="width:100%;border-collapse:collapse;font-size:12px">')
        w('<thead><tr style="background:#0d2040;border-bottom:2px solid #1e3a5f">')
        for _h in ["Ticker","Price","Status","Renko","Signal Date","Days In","EMA Comp","Squeeze","Brick","EMA 8","EMA 21","Score","Chart"]:
        ...
        for r in section_rows:
            ...
            if _fd and r.get("signal_active"):
                w(f'<td style="padding:6px 9px;text-align:center;font-size:11px;color:#FFD700;font-weight:700">{_fd}</td>')
            else:
                w('<td style="padding:6px 9px;text-align:center;color:#5a7fa0;font-size:11px">—</td>')
            # Days in trade column
            ...
            for _ev in [r.get("e8"), r.get("e21")]:
                _ec = "#00e676" if (_ev and r["px"]>_ev) else ("#ef5350" if _ev else "#555")
                w(f'<td style="padding:6px 9px;text-align:right;color:{_ec};font-size:11px">'
                  f'${_ev:,.2f}' if _ev else '<td style="color:#555;text-align:center;font-size:11px">—')
                w('</td>')
            _scr = r.get("score",0)
            ...
            w(f'<td style="padding:6px 9px;text-align:center">'
              ...
              f'style="color:#42a5f5;font-size:11px;font-weight:700">↗</a></td>')
            w('</tr>')
        w('</tbody></table></div>')
