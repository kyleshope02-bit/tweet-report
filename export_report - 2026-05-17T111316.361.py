...
def render_renko_tab(ohlcv_data: dict) -> str:
    ...
    def is_compressing(e8, e21, e34, i, lookback=3):
        """EMA spread shrinking over last N bars = compression."""
        if i < lookback+1: return False
        if any(e is None for arr in [e8,e21,e34] for e in [arr[i], arr[i-lookback]]): return False
        spread_now  = abs(e8[i]  - e34[i])
        spread_back = abs(e8[i-lookback] - e34[i-lookback])
        return spread_now < spread_back * 0.97  # narrowing by >3%
    ...
    for _t, _c, _d in [
        ...
        ("EMA Compression (8/21/34)", "#FFD700",
         "Matt Bishop strategy: when the 8, 21, and 34 EMAs converge toward each other, "
         "energy is building for a directional move. Compression = spread narrowing >3% over 3 bars."),
    ...
    for tk in MAG7:
        ...
        # Check flip (last 3 bricks: was red, now green)
        recent_bricks = bricks[-6:] if len(bricks) >= 6 else bricks
        had_red   = any(b["dir"]=="red"   for b in recent_bricks[:-1])
        now_green = bricks[-1]["dir"] == "green" if bricks else False
        renko_flip = had_red and now_green and bricks and bricks[-1].get("flip", False)

        # Recent flip (within last 5 bars even if not exact flip bar)
        recent_flip = (renko_dir == "green" and
                       any(b["dir"]=="red" for b in bricks[-8:-2]) if len(bricks) >= 3 else False)
        ...
        # Current EMA values
        e8_now  = round(e8_arr[-1],  2) if e8_arr[-1]  else None
        e21_now = round(e21_arr[-1], 2) if e21_arr[-1] else None
        e34_now = round(e34_arr[-1], 2) if e34_arr[-1] else None

        # Alert score
        score = sum([
            renko_dir == "green",   # Renko bullish
            renko_flip or recent_flip,  # recent flip
            comp,                   # EMA compressing
            sqz,                    # Squeeze active
        ])

        if score >= 3:
            status = "🚨 ALERT"; status_col = "#00e676"
        elif score == 2 and renko_dir == "green":
            status = "⚡ WATCH"; status_col = "#FFD700"
        elif renko_dir == "green":
            status = "🟢 BULLISH"; status_col = "#00e676"
        elif renko_dir == "red":
            status = "🔴 BEARISH"; status_col = "#ef5350"
        else:
            status = "NEUTRAL"; status_col = "#8ab4d4"

        alert_rows.append({
            "tk":tk,"col":col,"status":status,"status_col":status_col,
            "renko_dir":renko_dir,"flip":renko_flip or recent_flip,
            "compression":comp,"squeeze":sqz,"alert":score>=3,
            "bricks":bricks,"brick_size":round(brick_size,2),
            "px":px,"e8":e8_now,"e21":e21_now,"e34":e34_now,
            "score":score,
        })

    # Sort: alerts first, then watches, then bullish, then bearish
    _order = {"🚨 ALERT":0,"⚡ WATCH":1,"🟢 BULLISH":2,"🔴 BEARISH":3,"NEUTRAL":4,"NO DATA":5}
    alert_rows.sort(key=lambda x: _order.get(x["status"],4))

    # Alert count banner
    n_alerts = sum(1 for r in alert_rows if r["alert"])
    n_watch  = sum(1 for r in alert_rows if r["status"]=="⚡ WATCH")
    w('<div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap">')
    for _lbl, _cnt, _bc, _tc in [
        ("🚨 Active alerts",  n_alerts,               "#00e67633","#00e676"),
        ("⚡ Watching",       n_watch,                "#FFD70033","#FFD700"),
        ("Total stocks",      len(MAG7),              "#0d1b2a",  "#8ab4d4"),
    ]:
        w(f'<div style="background:{_bc};border-radius:8px;padding:10px 16px;text-align:center;min-width:120px">')
    ...
    # ── Main table ─────────────────────────────────────────────────────────────
    w('<div style="overflow-x:auto;margin-bottom:20px">')
    w('<table style="width:100%;border-collapse:collapse;font-size:12px">')
    w('<thead><tr style="background:#0d2040;border-bottom:2px solid #1e3a5f">')
    for _h in ["Ticker","Price","Status","Renko","Renko Flip","EMA Compress","Squeeze","Brick Size","EMA 8","EMA 21","EMA 34","Score","Chart"]:
        w(f'<th style="padding:7px 10px;color:#7eb8f7;font-weight:700;text-align:center;white-space:nowrap">{_h}</th>')
    w('</tr></thead><tbody>')

    for r in alert_rows:
        tv_url = f"{TV_BASE}?symbol={r['tk']}"
        _row_bg = "background:#051a0a;" if r["alert"] else (
                  "background:#0a1400;" if r["status"]=="⚡ WATCH" else "")
        w(f'<tr style="border-bottom:1px solid #0d2040;{_row_bg}">')
        w(f'<td style="padding:7px 10px;font-weight:900;color:{r["col"]};font-size:14px;text-align:center">{r["tk"]}</td>')
        w(f'<td style="padding:7px 10px;color:#e8edf5;font-weight:700;text-align:right">${r["px"]:,.2f}</td>')
        w(f'<td style="padding:7px 10px;text-align:center">'
          f'<span style="background:{r["status_col"]}22;color:{r["status_col"]};font-size:11px;font-weight:900;'
          f'padding:3px 10px;border-radius:5px">{r["status"]}</span></td>')
        # Renko direction
        _rd = r["renko_dir"] or "—"
        _rc = "#00e676" if _rd=="green" else ("#ef5350" if _rd=="red" else "#555")
        w(f'<td style="padding:7px 10px;text-align:center;color:{_rc};font-weight:800">'
          f'{"🟢 GREEN" if _rd=="green" else "🔴 RED" if _rd=="red" else "—"}</td>')
        # Flip
        _fc = "#00e676" if r.get("flip") else "#5a7fa0"
        w(f'<td style="padding:7px 10px;text-align:center;color:{_fc};font-weight:700">'
          f'{"✅ YES" if r.get("flip") else "—"}</td>')
        # EMA compression
        _cc = "#FFD700" if r.get("compression") else "#5a7fa0"
        w(f'<td style="padding:7px 10px;text-align:center;color:{_cc};font-weight:700">'
          f'{"🌀 YES" if r.get("compression") else "—"}</td>')
        # Squeeze
        _sc = "#42a5f5" if r.get("squeeze") else "#5a7fa0"
        w(f'<td style="padding:7px 10px;text-align:center;color:{_sc};font-weight:700">'
          f'{"🔥 YES" if r.get("squeeze") else "—"}</td>')
        w(f'<td style="padding:7px 10px;text-align:center;color:#8ab4d4">${r["brick_size"]:,.2f}</td>')
        # EMAs
        for _ev in [r.get("e8"), r.get("e21"), r.get("e34")]:
            _px = r["px"]
            _ec = "#00e676" if (_ev and _px > _ev) else ("#ef5350" if _ev else "#555")
            w(f'<td style="padding:7px 10px;text-align:right;color:{_ec}">'
              f'${_ev:,.2f}' if _ev else '<td style="color:#555;text-align:center">—')
            w('</td>')
        # Score
        _scr = r.get("score", 0)
        _sc2 = "#00e676" if _scr>=3 else ("#FFD700" if _scr==2 else "#8ab4d4")
        w(f'<td style="padding:7px 10px;text-align:center;font-size:15px;font-weight:900;color:{_sc2}">{_scr}/4</td>')
        # Chart link
        w(f'<td style="padding:7px 10px;text-align:center">'
          f'<a href="{tv_url}" target="_blank" '
          f'style="background:#185FA522;color:#42a5f5;font-size:11px;font-weight:700;'
          f'padding:4px 10px;border-radius:5px;text-decoration:none">↗ TV</a></td>')
        w('</tr>')

    w('</tbody></table></div>')

    # ── Per-stock Renko brick visualisation ────────────────────────────────────
    w('<div style="font-size:13px;font-weight:900;color:#FFD700;margin-bottom:12px">📊 Renko Brick History — last 40 bricks per stock</div>')
    w('<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:16px">')

    for r in alert_rows:
        if not r.get("bricks") or r["px"] == 0:
            continue
        col    = r["col"]
        bricks = r["bricks"][-40:]  # last 40 bricks
        tk     = r["tk"]
